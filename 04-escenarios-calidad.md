...
### ESC-PERF-03 — Comportamiento bajo carga de estrés (500+ usuarios concurrentes)
 
| Elemento                | Descripción                                                                                                                                                    |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Fuente**              | Población simulada de estudiantes (carga de estrés, no uso típico)                                                                                              |
| **Estímulo**            | 500 o más usuarios virtuales intentan iniciar sesión de forma concurrente en una ventana corta de tiempo.                                                      |
| **Entorno**             | Servidor de desarrollo Flask (`debug=True`, `threaded=True`), MySQL local, single-machine — no representa infraestructura de producción.                       |
| **Artefacto**           | Ruta `/loginAccess` (autenticación + 7 consultas a `recordatorios` + inserción de traza + render de `admin.html`).                                             |
| **Respuesta**           | El sistema debe degradar de forma predecible (tiempos de respuesta crecientes, sin caídas totales del proceso) en vez de fallar de forma abrupta o silenciosa. |
| **Medida de respuesta** | Tasa de error < 5% sostenida durante la carga; el sistema debe seguir respondiendo (aunque más lento) sin que el proceso de Flask se caiga por completo.       |
 
*(Nota metodológica: a diferencia de ESC-PERF-01/02, aquí NO se espera necesariamente cumplir un umbral de latencia estricto — el objetivo es observar el punto de quiebre real del sistema base, que es información arquitectónica valiosa en sí misma. Ver `/experimentos/metodo.md` para las limitaciones ya documentadas de correr generador de carga y servidor en la misma máquina, agravadas a esta escala.)*
 
Fila para la tabla resumen (sección 7):
 
| ID | Atributo | Escenario | Prioridad |
|---|---|---|---|
| ESC-PERF-01 | Performance | Tiempo de respuesta bajo carga concurrente (10 VUs) | Alta |
| ESC-PERF-02 | Performance | Login y carga del menú principal (10 VUs) | Alta |
| ESC-PERF-03 | Performance | Comportamiento bajo carga de estrés (500+ VUs) | Alta |
